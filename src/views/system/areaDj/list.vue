...
const columns = ref([{"title":"父级code","dataIndex":"parentCode","ellipsis":true,"align":"left"},{"title":"省市区县名","dataIndex":"areaName","ellipsis":true,"align":"left"},{"title":"省市区县","dataIndex":"areaCode","ellipsis":true,"align":"left"},{"title":"状态","dataIndex":"statusEnum","ellipsis":true,"align":"left"},{"title":"是否修改","dataIndex":"isAddEnum","ellipsis":true,"align":"left"},{"title":"省市区","dataIndex":"typeEnum","ellipsis":true,"align":"left"},{"title":"拼音","dataIndex":"pinyin","ellipsis":true,"align":"left"},{"title":"操作","dataIndex":"action","ellipsis":true,"align":"left","width":90,"fixed":"right"}]);
...
async function f5() {
    ...
    try {
        let queryResult = await base.put('/areaDj/listPage', p);
...
function exportFile() {
    base.download("/areaDj/export", p.value)
}
...
function del(data) {
    base.confirm('是否删除，此操作不可撤销', async function () {
        let res = await base.delete("/areaDj/delete/" + data.id);
...
async function updateSwitch(row) {
    await base.post('/areaDj/save', row);
    base.success("更新成功")
}
