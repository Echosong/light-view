<template>
    ...
    <a-form class="smart-query-form">
        <a-row class="smart-query-form-row">
            <a-form-item label="用户账号" v-if="!query.userAccount" class="smart-query-form-item">
 <a-input v-model:value="p.userAccount" placeholder="模糊查询"></a-input>
</a-form-item>
...
const columns = ref([{"title":"用户账号","dataIndex":"userAccount","ellipsis":true,"align":"left"},{"title":"图片","dataIndex":"images","ellipsis":true,"align":"left"},{"title":"内容","dataIndex":"content","ellipsis":true,"align":"left"},{"title":"联系方式","dataIndex":"contact","ellipsis":true,"align":"left"},{"title":"是否已读","dataIndex":"isRead","ellipsis":true,"align":"left"},{"title":"意见反馈问题分类","dataIndex":"categoryEnum","ellipsis":true,"align":"left"},{"title":"操作","dataIndex":"action","ellipsis":true,"align":"left","width":90,"fixed":"right"}]);

// ---------------------------- 查询数据表单和方法 ----------------------------

const params = {pageSize:10,page:1, total: 0, userAccount:''}
// 查询表单form
...
async function f5() {
    ...
    try {
        let queryResult = await base.put('/feedback/listPage', p);
...
function exportFile() {
    base.download("/feedback/export", p.value)
}
...
function del(data) {
    base.confirm('是否删除，此操作不可撤销', async function () {
        let res = await base.delete("/feedback/delete/" + data.id);
...
async function updateSwitch(row) {
    await base.post('/feedback/save', row);
    base.success("更新成功")
}
