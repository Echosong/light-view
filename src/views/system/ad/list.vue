<template>
    ...
    <a-form class="smart-query-form">
        <a-row class="smart-query-form-row">
            <a-form-item label="位置"  v-if="!query.position" class="smart-query-form-item">
      <input-enum enumName="adPositionEnum"  style="width: 180px;"  v-model="p.position"></input-enum>
</a-form-item>
<a-form-item label="名称" v-if="!query.name" class="smart-query-form-item">
...
const columns = ref([{"title":"位置","dataIndex":"positionEnum","ellipsis":true,"align":"left"},{"title":"名称","dataIndex":"name","ellipsis":true,"align":"left"},{"title":"关联代码","dataIndex":"relationCode","ellipsis":true,"align":"left"},{"title":"图片","dataIndex":"image","ellipsis":true,"align":"left"},{"title":"跳转位置","dataIndex":"url","ellipsis":true,"align":"left"},{"title":"状态","dataIndex":"stateEnum","ellipsis":true,"align":"left"},{"title":"关联的实体","dataIndex":"info","ellipsis":true,"align":"left"},{"title":"操作","dataIndex":"action","ellipsis":true,"align":"left","width":90,"fixed":"right"}]);

// ---------------------------- 查询数据表单和方法 ----------------------------

const params = {pageSize:10,page:1, total: 0, position:'',name:''}
// 查询表单form
...
async function f5() {
    ...
    try {
        let queryResult = await base.put('/ad/listPage', p);
...
function del(data) {
    base.confirm('是否删除，此操作不可撤销', async function () {
        let res = await base.delete("/ad/delete/" + data.id);
