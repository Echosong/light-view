...
async function f5() {
    ...
    try {
        let queryResult = await base.put('/dictData/listPage', p);
...
function exportFile() {
    base.download("/dictData/export", p.value)
}
...
function del(data) {
    base.confirm('是否删除，此操作不可撤销', async function () {
        let res = await base.delete("/dictData/delete/" + data.id);
...
async function updateSwitch(row) {
    await base.post('/dictData/save', row);
    base.success("更新成功")
}
