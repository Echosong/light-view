...
<a-form-item label="访问路径"  prop="url" v-if="!query.url" >
...
<a-form-item label="父级id"  prop="parentId" v-if="!query.parentId" >
    <a-input v-model:value="m.parentId"></a-input>
</a-form-item>
<a-form-item label="类型"  prop="type" v-if="!query.type" tooltip="权限类型 1：目录   2：菜单   3：按钮">
    <a-input v-model:value="m.type"></a-input>
</a-form-item>
...
<a-form-item label="是否展现 1 展现 0 不展现"  prop="show" v-if="!query.show" >
    <a-input v-model:value="m.show"></a-input>
</a-form-item>
        </a-form>
        <template #footer>
            <a-space>
...
async function open(data, parmas)  {
    ...
    if (data) {
        title.value = "修改 权限";
        let one = await base.get("/permission/find/"+data.id);
        m.value = one.data;
        m.value = data;
    } else {
        let mdata  = {name:'',
