<template>
  <a-layout class="admin-layout" style="min-height: 100%">
    ...
    <a-layout id="LightAdminMain" :style="`height: ${windowHeight}px`" class="admin-layout-main">
      <!-- 顶部头部信息 -->
      ...
      <!--中间内容-->
      <a-layout-content id="LightAdminLayoutContent" class="admin-layout-content">
...
<script setup>
  ...
  onMounted(() => {
    if (watermarkFlag.value) {
      watermark.set('LightAdminLayoutContent', useUserStore().actualName);
    } else {
      watermark.clear();
    }
  });
  ...
  //回到顶部
  const backTopTarget = () => {
    return document.getElementById('LightAdminMain');
  };
